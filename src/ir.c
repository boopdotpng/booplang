...
instruction *emit_store() {
  return emit_instruction(OP_STORE);
}
instruction *emit_load() {
  return emit_instruction(OP_LOAD);
}
instruction *emit_phi() {
  return emit_instruction(OP_PHI);
}
instruction *emit_print() {
  return emit_instruction(OP_PRINT);
}
instruction *emit_call() {
  return emit_instruction(OP_CALL);
}
...
int parse_expression(ast_node *node, block *current, func *fn) {
  ...
  default: fprintf(stderr, "unsupported expression node type: %d\n", node->type); exit(1);
  }
}
...
void parse_statement(ast_node *node, block **current, func *fn) {
  ...
  case NODE_RETURN: {
    int ret_reg = -1;
    if (node->left) ret_reg = parse_expression(node->left, *current, fn);
  ...
  case NODE_IF: {
    ...
    *current = then_blk;
    ...
    if (node->else_body) add_element(fn->blocks, &else_blk);
  ...
  case NODE_FOR: {
    ...
    *current = body;
    ...
    if (node->step) parse_statement(node->step, current, fn);
    instruction *jmp_back = emit_instruction(OP_JMP);
    add_element(jmp_back->args, &hdr_arg);
    add_element((*current)->instructions, &jmp_back);
  ...
  default: fprintf(stderr, "unsupported statement node type: %d\n", node->type); exit(1);
  }
}
...
ir_module *gen_ir(const char *filename, int write_file, ast_node *program) {
  ...
  if (write_file) ir_to_file("ir.boopir", m);
  return m;
}
